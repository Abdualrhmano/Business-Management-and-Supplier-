...
(function () {
  ...
  if (form) {
    ...
    // حفظ تلقائي كل 2 ثانية بعد آخر تغيير
    let saveTimer = null;
  ...
  // نبحث عن زر الإرسال الأصلي ونغلفه بوظائف إضافية (spinner, toasts, retry)
  if (form) {
    ...
    if (originalSubmit) {
      ...
      form.addEventListener('submit', async (ev) => {
        ...
        // ننتظر قليلاً ثم نتحقق إن كان الكود الأصلي قد أجرى fetch عبر /api/generate
        // إذا لم يستخدم fetch أو استخدم endpoint آخر، لا نتدخل.
        // نراقب الشبكة عبر استبدال مؤقت لـ window.fetch (غير دائم) فقط أثناء المعالجة
        const nativeFetch = window.fetch;
        let usedOurFetch = false;
        window.fetch = async function (...args) {
          // إذا كان الطلب إلى /api/generate نستخدم retry ذكي
          try {
            const url = args[0];
            ...
            // لا نعيد window.fetch هنا لأن قد يكون هناك طلبات لاحقة؛ سنعيده في finally أدناه
          }
        };

        // ننتظر انتهاء معالجة الكود الأصلي: نراقب إزالة analysisUI أو ظهور planOutput كإشارة
        // وضع حد أقصى للانتظار
        const waitForCompletion = () => new Promise((resolve) => {
          ...
          const check = () => {
            ...
            // إشارة نجاح: planOutput مرئي أو analysisUI مخفي
            if ((planOutput && !planOutput.classList.contains('hidden')) || (analysisUI && analysisUI.classList.contains('hidden'))) {
              resolve({ success: true, usedOurFetch });
              return;
            }
            if (Date.now() - start > maxWait) {
              resolve({ success: false, usedOurFetch });
              return;
            }
        ...
        try {
          ...
          // إعادة fetch الأصلي
          try { window.fetch = nativeFetch; } catch (e) { /* تجاهل */ }
  ...
  // (نترك الكود الأصلي لطباعة كما هو؛ نضيف فقط رسالة عند فشل فتح النافذة)
  // نراقب click على زر الطباعة إن وُجد
  const printBtn = document.getElementById('print-btn');
  if (printBtn) {
    printBtn.addEventListener('click', () => {
      setTimeout(() => {
        // إذا لم تُفتح نافذة جديدة خلال 800ms، نعرض تلميح
        // لا يمكننا كشف النافذة المفتوحة من الكود الأصلي، لذا نعرض تلميح عام
        // (هذا لا يغيّر سلوك الطباعة الأصلي)
